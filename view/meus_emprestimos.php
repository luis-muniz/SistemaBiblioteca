...
<body>
    ...
    <div class="row container">
        ...
        <tbody>
            <?php 
                $linhas = consultarEmprestimos($_SESSION['user']);
                foreach ($linhas as $linha) {
                    echo '<tr>'
                            .'<td>'.$linha["emprestimo_id"].'</td>'
                            .'<td>'.$linha["exemplar_id"].'</td>'
                            .'<td>'.$linha["titulo"].'</td>'
                            .'<td>'.$linha["data_vencimento"].'</td>'
                            .'<td>'.$linha["situacao"].'</td>'
                            .'<td>'.$linha["total_renovacoes"].'/10</td>'
                            .'<td><a class="btn" href="../php/renovar_emprestimo.php?emprestimoID='.$linha["emprestimo_id"].'">Renovar</a></td>'
                        .'</tr>';
                }
            ?>
        </tbody>
